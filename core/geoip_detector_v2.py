# -*- coding: utf-8 -*-
"""
Clash Config Auto Builder - 地理位置检测器 V2
使用自建IP检测服务器获取代理节点出口IP，然后进行地理位置识别
"""
...
class GeoIPDetectorV2:
    """地理位置检测器 V2 - 使用自建IP检测服务器"""
    
    def __init__(self, ip_server_url: str = "http://localhost:8080"):
        self.logger = setup_logger("geoip_detector_v2")
        self.ip_server_url = ip_server_url.rstrip('/')
    ...
    def test_ip_server_connection(self) -> bool:
        """测试IP检测服务器连接"""
        try:
            response = requests.get(f"{self.ip_server_url}/health", timeout=5)
            if response.status_code == 200:
                self.logger.info(f"IP检测服务器连接正常: {self.ip_server_url}")
                return True
            else:
                self.logger.error(f"IP检测服务器响应异常: {response.status_code}")
                return False
        except Exception as e:
            self.logger.error(f"无法连接到IP检测服务器 {self.ip_server_url}: {e}")
            return False
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if self._is_valid_ip(ip):
                    self.logger.info(f"代理 '{proxy_name}' 出口IP: {ip}")
                    return ip
                else:
                    self.logger.warning(f"代理 '{proxy_name}' 返回无效IP: {ip}")
                    return None
            else:
                self.logger.warning(f"代理 '{proxy_name}' IP检测失败: HTTP {response.status_code}")
                return None
    ...
    def detect_and_rename_nodes(self, proxies: list, api_url: str = "127.0.0.1:9090",
                               max_workers: int = 5, timeout: int = 20) -> list:
        """
        批量检测节点地理位置并重新命名
        ...
        """
        # 首先测试IP检测服务器连接
        if not self.test_ip_server_connection():
            self.logger.error("IP检测服务器不可用，跳过地理位置检测")
            return proxies
