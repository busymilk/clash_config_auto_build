# -*- coding: utf-8 -*-
"""
Clash Config Auto Builder - 地理位置检测器
通过测试节点出口IP来准确识别地区并重新命名节点
"""
...
class GeoIPDetector:
        ...
        try:
            ...
            for service in ip_services:
                try:
                    ...
                    self.logger.debug(f"IP服务 {service} 失败: {e}")
                    continue
    ...
    def detect_and_rename_nodes(self, proxies: list, api_url: str = "127.0.0.1:9090",
                               max_workers: int = 10, timeout: int = 15) -> list:
        """
        批量检测节点地理位置并重新命名
        ...
        self.logger.info(f"地理位置检测完成！成功重命名 {len([p for p in renamed_proxies if p.get('_renamed')])} 个节点")
