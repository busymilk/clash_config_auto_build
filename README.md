...
- **两阶段健康检查**: 
    1.  **国内可达性测试**: 独创的“自测”并发模型。通过每个代理节点自身，去请求在线TCPing网站，由其位于中国大陆的探测点进行TCP端口测试。这确保了我们能筛选出真正能从国内访问的节点，并有效规避了IP封锁。
    2.  **延迟与速度测试**: 对通过了第一阶段测试的节点，再进行传统的并发延迟测试，确保最终选出的节点既能连上，又速度快。
