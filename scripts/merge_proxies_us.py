...
name_pattern = re.compile(
    r'\b(us|usa|america|united[\s-]?states)\b(?!-?(check|fail))|美|🇺🇸',  # 优化后的美国关键词正则（含Emoji）
    flags=re.IGNORECASE
)
        ...
        with open(file, 'r', encoding="utf-8") as f:
            ...
            for proxy in file_proxies:
                ...
                if not name or not name_pattern.search(name):
                    logging.info(f"排除代理 {name}：名称不包含美国关键词")
                    continue
                ...
                if all(identifier) and identifier not in seen:
                    ...
                    logging.info(f"添加美国代理: {name} | 类型: {proxy_type} | 服务器: {server}:{port}")
                else:
                    # 如果节点是重复的或关键信息不完整，则记录并跳过
                    if all(identifier):
                        logging.info(f"排除重复的美国代理: {name} | 类型: {proxy_type} | 服务器: {server}:{port}")
                    else:
                        logging.warning(f"排除信息不完整的美国代理: {name}")
